...
#pragma region Exists Tests
  ...
  TEST_METHOD(File_Exists_ShouldReturnFalse)
  {
    ...
    Assert::IsFalse(File::exists(Directory::getExecutingAppDirectory()));
  }
...
#pragma region Create Tests
  ...
  {
    Path parent(TempDirectory::getFullPath(), "Nested");
...
#pragma region Copy String Input Tests
  ...
  {
    File file(Path(TempDirectory::getFullPath(), "WubbaLubbaDubDub"));

    Assert::IsFalse(file.exists());

    Path dest(TempDirectory::getFullPath(), "Test.txt");
    file.copy(dest.as_string());

    FileAssert::FileDoesNotExist(dest);
  }

  //------------------------------------------------------------------------------------------------
  TEST_METHOD(File_Copy_StringInput_DestinationFilePathIsInvalid_DoesNothing)
  {
    File file(Path(TempDirectory::getFullPath(), "Data.xml"));
    file.append("Testing Testing 123");

    Assert::IsTrue(file.exists());

    Path dest(TempDirectory::getFullPath(), "WubbaLubba<<!_$DubDub");
    file.copy(dest.as_string());
  ...
  TEST_METHOD(File_Copy_StringInput_WithFilePathThatDoesNotExist_CreatesFileWithSameContents)
  {
    ...
    FileAssert::FileExists(dest);
    FileAssert::FileContentsEqual(dest.as_string(), "Testing Testing 123");
  }
  ...
  TEST_METHOD(File_Copy_StringInput_WithFilePathThatDoesExist_DoesNothing)
  {
    ...
    File dest(Path(TempDirectory::getFullPath(), "Dest.txt"));
    dest.append("Hello World");
    
    FileAssert::FileExists(dest.getFilePath().as_string());
    
    file.copy(dest.getFilePath().as_string());

    Assert::IsTrue(dest.exists());
    FileAssert::FileContentsEqual(dest.getFilePath().as_string(), "Hello World");
  }
...
#pragma region Copy Path Input Tests
  ...
  TEST_METHOD(File_Copy_PathInput_WithFilePathThatDoesExist_DoesNothing)
  {
    ...
    FileAssert::FileExists(dest.getFilePath());
...
#pragma region Append Tests
  ...
  TEST_METHOD(File_Append_FileDoesntExist_CreatesFileToo)
  {
    ...
    File file(Path(TempDirectory::getFullPath(), "Directory1", "Directory2", "Test.txt"));

    Assert::IsFalse(Directory(Path(TempDirectory::getFullPath(), "Directory1", "Directory2")).exists());
    Assert::IsFalse(file.exists());
